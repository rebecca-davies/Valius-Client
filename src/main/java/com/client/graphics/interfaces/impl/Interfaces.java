...
public class Interfaces extends RSInterface {
	...
	public static void loadInterfaces() {
		...
		teamIronmanChat(defaultTextDrawingAreas);
	...
	/*	int[] spriteIds = { 0, 2, 4, 6, 8, 11, 14, 17, 20, 23, 25, 29, 31, 33, 36, 37 };
		Sprite[] backgroundSprites = new Sprite[16];
		for(int index = 0;index<spriteIds.length;index++) {
			int spriteId = spriteIds[index];
			backgroundSprites[index] =  RSInterface.method207(spriteId, archive, "magicicon");
		}
		int[] ids = {24111, 24112, 24113, 24114, 24115, 24116};
		for(int id : ids)
			interfaceCache[id].backgroundSprites = backgroundSprites;*/
	...
	public static void normals(TextDrawingArea[] tda) {
		...
		RSInterface zenyte = RSInterface.interfaceCache[23649];
		
		zenyte.anIntArray212 = new int[] {19, 19, 0, 92};
	...
	public static void ironmanWidget(TextDrawingArea[] tda) {
		...
		addText(42409, "Create or join a team of up to 4 other players that you can trade with.", tda, 0, 0xFD851A, false, true);
		addText(42424, "    Hardcore Iron Man accounts only have 1 life.",tda, 0, 0xFD851A, false, true);
		addText(42410, "No Iron man restrictions will apply to this account.", tda, 0,
				0xFD851A, false, true);
		addText(42411, "Speak to the Ironman tutor to remove Ironman status.", tda, 0, 0xFD851A, false, true);
		...
		addText(42414, "Team Iron Man", tda, 0, 0xFFFFFF, false, true);
		addText(42415, "None", tda, 0, 0xFFFFFF, false, true);
		addText(42416, "NPC Reset", tda, 0, 0xFFFFFF, false, true);

		addText(42417, "Iron Man Mode", tda, 1, 0xFFFFFF, false, true);
		addText(42418, "Other Options...", tda, 1, 0xFFFFFF, false, true);
		...
		setBounds(42409, 50, 187, 8, widget); // Reg mode text
		setBounds(42424, 39, 166, 22, widget); // hcim mode text

		setBounds(42410, 130, 248, 9, widget);
		...
		setBounds(42422, 50, 155, 20, widget); // OSRS Mode

		setBounds(42414, 50, 177, 13, widget);
	...
	public static void slayerInterfaceSub1(TextDrawingArea[] tda) {
		...
		addTextButton(41514, "Learn how to encounter Superior Slayer NPCS				  (250)", "Learn", 0xFF981F,
				false, true, tda, 1, 406);
		addTextButton(41515, "Learn to fight Ice strykewyrms without a Fire/Infernal/Tokhaar cape				  (200)", "Learn", 0xFF981F,
		...
		rsInterface.child(9, 41514, 67, 165);
		rsInterface.child(10, 41515, 67, 185);
	}
	...
	public static void clanChatTab(TextDrawingArea[] tda) {
		...
		addHoverButton(18125, "/clanchat/teamim", 9, 15, 15, "Switch to ironman team chat", 550, 18126, 5);//team ironman button
		addHoveredButton(18126, "/clanchat/teamim", 10, 15, 15, 18127);//team ironman button
		...
		tab.child(13, 18125, 165, 38);
		tab.child(14, 18126, 165, 38);
		/* Text area */
	...
	public static void teamIronmanChat(TextDrawingArea[] tda) {
		RSInterface tab = addTabInterface(18500);
		addHoverButton(18501, "/clanchat/sprite", 6, 72, 32, "Invite Member", 550, 18502, 5);
		addHoveredButton(18502, "/clanchat/sprite", 7, 72, 32, 18503);
		addHoverButton(18504, "/clanchat/sprite", 6, 72, 32, "Leave Team", -1, 18505, 5);
		addHoveredButton(18505, "/clanchat/sprite", 7, 72, 32, 18506);
		addText(18507, "Invite Member", tda, 0, 0xff9b00, true, true);
		addText(18508, "Leave Team", tda, 0, 0xff9b00, true, true);
		addSprite(18509, 37, "/clanchat/sprite");
		addText(18510, "Ironman Team", tda, 1, 0xff9b00, true, true);
		addText(18511, "Team Name: Not in a Team", tda, 0, 0xff9b00, false, true);
		addText(18512, "Team Leader: None", tda, 0, 0xff9b00, false, true);
		addHoverButton(18497, "/clanchat/teamim", 9, 15, 15, "Switch to clan chat", 550, 18498, 5);//team ironman button
		addHoveredButton(18498, "/clanchat/teamim", 10, 15, 15, 18499);//team ironman button
		addSprite(16513, 4, "/clanchat/sprite");
		tab.totalChildren(15);
		tab.child(0, 16513, 0, 221);
		tab.child(1, 16513, 0, 59);
		tab.child(2, 18501, 0, 62);
		tab.child(3, 18502, 0, 62);
		tab.child(4, 18503, 15, 226);
		tab.child(5, 18504, 15, 226);
		tab.child(6, 18505, 103, 226);
		tab.child(7, 18507, 103, 226);
		tab.child(8, 18508, 51, 237);
		tab.child(9, 18509, 139, 237);
		tab.child(10, 18510, 95, 1);
		tab.child(11, 18511, 10, 23);
		tab.child(12, 18512, 25, 38);
		tab.child(13, 18497, 165, 38);
		tab.child(14, 18498, 165, 38);
		/* Text area */
		RSInterface list = addTabInterface(18643);
		list.totalChildren(100);
		for (int i = 18644; i <= 18744; i++) {
			addText(i, "", tda, 0, 0xffffff, false, true);
		}
		for (int id = 18644, i = 0; id <= 18743 && i <= 99; id++, i++) {
			interfaceCache[id].actions = new String[] { "Edit Rank", "Kick Member", "Ban Member" };
			list.children[i] = id;
			list.childX[i] = 5;
			for (int id2 = 18644, i2 = 1; id2 <= 18743 && i2 <= 99; id2++, i2++) {
				list.childY[0] = 2;
				list.childY[i2] = list.childY[i2 - 1] + 14;
			}
		}
